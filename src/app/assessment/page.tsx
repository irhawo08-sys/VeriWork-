...
import { Clock, FileText, Users, Upload } from "lucide-react";
  ...
  return (
    <>
        ...
        <div className="section grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          ...
          <div className="grid gap-4">
            {[
              ...
              { icon: FileText, text: "Supporting evidence upload is optional but strengthens review" },
      ...
      <section className="section max-w-3xl">
        ...
        />
        <form action={formAction} encType="multipart/form-data" className="mt-8 flex flex-col gap-8">
          ...
          <div>
            <SectionHeading
              eyebrow="Supporting evidence (optional)"
              title="Upload one document to strengthen your review"
              description="A pay structure, safety policy, or grievance process document. This can also be sent separately later — it's optional now."
            />
            <div className="mt-6 flex items-center gap-4 rounded-xl2 border border-dashed border-forest/25 bg-white p-6">
              <Upload className="shrink-0 text-forest" size={22} />
              <div className="flex-1">
                <label htmlFor="evidence" className="text-sm font-medium text-forest">Choose a file</label>
                <input
                  id="evidence"
                  name="evidence"
                  type="file"
                  accept=".pdf,.doc,.docx,.jpg,.jpeg,.png"
                  className="mt-2 block w-full text-sm text-ink/70 file:mr-4 file:rounded-full file:border-0 file:bg-forest/10 file:px-4 file:py-2 file:text-sm file:font-medium file:text-forest"
                />
                <p className="mt-2 text-xs text-ink/50">PDF, Word, or image — up to a few MB.</p>
              </div>
            </div>
          </div>
