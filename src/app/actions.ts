...
export async function submitAssessment(
  ...
  if (evidenceFile && evidenceFile.size > 0) {
    ...
    const filePath = `${assessment.id}/${Date.now()}_${safeName}`;
  ...
  return {
    status: "success",
    message: evidenceUploaded
      ? "Assessment submitted successfully with supporting evidence! Your platform now has an 'In Progress' profile in the public directory."
      : "Assessment submitted successfully! Your platform now has an 'In Progress' profile in the public directory.",
  };
}
